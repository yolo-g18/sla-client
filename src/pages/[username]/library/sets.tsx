...
const sets = (props: any) => {
  const { auth, user, alert } = useSelector((state: RootStore) => state);
    ...
    const fetchData = async () => {
      ...
      const fetchCreatedSets = async () => {
        try {
          ...
          if (listSSCreatedRes.data.length === 0) setIsShowEmptyCreatedSS(true);
          else setIsShowEmptyCreatedSS(false);
      ...
      const fetchLearningSets = async () => {
        try {
          ...
          if (listSSLearningRes.data.length === 0)
            setIsShowEmptyLearningSS(true);
          else setIsShowEmptyLearningSS(false);
