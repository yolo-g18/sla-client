...
const LayoutBase = ({
  ...
  // );
  const errors: any = React.useContext(MyContext);
  console.log("basicLayout_errors", errors);
  console.log("data form is: " + appointmentData.id);
  return (
    <div
      ...
      {...restProps}
    >
      <Label text={getMessage("detailsLabel")} />
      <TextEditor
      ...
      {resources.map((resource: any) => (
        <React.Fragment key={resource.fieldName}>
          <Label text={resource.title} className={classes.labelWithMargins} />
          <ResourceEditor
            readOnly={readOnly}
            resource={resource}
